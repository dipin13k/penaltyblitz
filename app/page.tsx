...
function initGame() {
  ...
  function applyTheme(light: boolean) {
    ...
    const btn = document.getElementById('themeToggleBtn') as HTMLElement | null;
  ...
  let cachedLeaderboardData: any[] | null = null;
  ...
  (window as any).forceRefreshLeaderboard = () => { cachedLeaderboardData = null; loadLeaderboard(); };
...
<div style="display:flex;align-items:center;padding:10px 16px;gap:12px;border-bottom:1px solid #222;background:${i === 0 ? '#1e2a1e' : i === 1 ? '#1e1e2a' : i === 2 ? '#2a1e1e' : 'transparent'};">
  ...
  async function showProfileScreen() {
    ...
    const bgStyle = isLight ? 'background:linear-gradient(135deg,#e8eaf6 0%,#dde3f0 100%);color:#111;' : 'background:linear-gradient(135deg,#0d0d1a 0%,#1a1a2e 100%);color:white;';
    ...
    <div id="screen-profile" style="position:fixed;inset:0;${bgStyle}display:flex;flex-direction:column;align-items:center;padding:40px 20px 80px;overflow-y:auto;font-family:sans-serif;z-index:500;">
      ...
      <div style="width:100%;max-width:320px;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
        ...
        ] as [string, string | number, string][]).map(([l, v, ic]) => `
